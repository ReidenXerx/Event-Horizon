...
| [`INSTALL_ACTION.md`](INSTALL_ACTION.md) | "Install Event Horizon Collection" — file pick → `readEhcoll` → `readReceipt` → snapshot pipeline → `buildUserSideState` → `pickInstallTarget` → `resolveInstallPlan` → preview dialog → (slice 6a) `runInstall` for fresh-profile installable plans, with progress notification + result dialog. |
