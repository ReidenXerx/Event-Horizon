...
| Build fails listing files that "are archives" | Archive files are left out of the mods a build ships; one reached packaging anyway because it changed after the build read its mod | Rebuild. |
| A bundled or mirrored mod is missing a file for users | The file is itself an archive (.zip, .7z, a .docx, a .jar…), which a package cannot carry, so the build left it out; the event-horizon log lists it under `build.archives-left-out` | If the mod needs what is inside, extract it into the mod's folder and rebuild. |
